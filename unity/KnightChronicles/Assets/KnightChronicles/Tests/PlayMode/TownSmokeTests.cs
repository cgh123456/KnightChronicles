...
{
    /// <summary>
    /// 冒险者小镇冒烟测试：验证玩家生成、输入驱动移动、八方向朝向切换、
    ...
    public sealed class TownSmokeTests
    {
        ...
        public IEnumerator Town_PlayerMovesAndFacesMovementDirection()
        {
            ...
            var animator = player.GetComponent<KnightChronicles.Runtime.SpriteAnimator>();
            Assert.IsNotNull(animator, "玩家应有八方向图集动画器");
            ...
            for (var i = 0; i < 30; i++)
            {
                // Manual physics simulation does not invoke MonoBehaviour.FixedUpdate.
            ...
            Assert.AreEqual("walk",animator.CurrentAction,"移动时应播放行走帧");
            Assert.AreEqual(2, animator.Direction, "向右移动应面向 E");
            ...
            Physics2D.Simulate(Time.fixedDeltaTime);
            yield return null;
            Assert.AreEqual(4, animator.Direction, "向上移动应面向 N");

            controller.InputProvider = () => Vector2.left;
            yield return null;
            Physics2D.Simulate(Time.fixedDeltaTime);
            yield return null;
            Assert.AreEqual(6, animator.Direction, "向左移动应面向 W");
            ...
            Assert.AreEqual("idle",animator.CurrentAction,"静止时应播放待机帧");
